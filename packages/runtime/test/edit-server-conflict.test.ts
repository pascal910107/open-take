...
// <base>.take/composition.json — the editor autosaves every ~700ms while the agent
...
import { type TakePaths, resolveTakePaths } from "../src/take.js";
...
let dir: string;
let paths: TakePaths;
/** the take's composition — inside its `.take/` working dir, where both
 *  writers (this editor and the agent) contend for it */
let compPath: string;
...
async function agentWrites(patch: (c: Record<string, unknown>) => void): Promise<number> {
  ...
  patch(comp);
  await writeFile(compPath, JSON.stringify(comp, null, 2));
  return (await stat(compPath)).mtimeMs;
}

before(async () => {
  dir = await mkdtemp(join(tmpdir(), "open-take-editsrv-"));
  paths = await resolveTakePaths(join(dir, "demo.mp4"));
  compPath = paths.compositionPath;
  await mkdir(paths.dir, { recursive: true });
  await writeFile(compPath, JSON.stringify(planComposition(log), null, 2));
  await writeFile(paths.captureLogPath, JSON.stringify(log));
  await writeFile(paths.capturePath, ""); // only stat'd unless /video is fetched
  server = await startEditServer(
    { takePath: paths.mp4Path, port: 0, open: false },
    {
...
test("GET /api/take hands out the mtime its composition was read at", async () => {
  ...
  assert.equal(take.mtime, (await stat(compPath)).mtimeMs);
});
...
test("a save on the current base is accepted and re-bases the client", async () => {
  ...
  assert.equal(body.mtime, (await stat(compPath)).mtimeMs);
...
test("a save over an unseen agent write is refused with 409 — and the agent's edit survives", async () => {
  ...
  const onDisk = JSON.parse(await readFile(compPath, "utf8"));
...
test("an agent write while the server prepares its temp file is caught before rename", async () => {
  ...
  try {
    ...
    assert.equal(
      await stat(`${compPath}.tmp`).catch(() => null),
      null,
